...
contract TestEndToEnd is Test, IManagerEvents, IRateLimiterEvents {
    ...
    function test_chain_to_chain() public{
        ...
        // 0x0000000000000000000000000f8458e544c9d4c7c25a881240727209caae20b8
        ...
        // Tamper with VAA at every byte with every other possible value
        // bytes memory oldEncodedVM0 = copyBytes(encodedVMs[0]);
        // console.log(encodedVMs[0].length);
        // for (uint i = 0; i < encodedVMs[0].length; i++){
        //     for(uint8 j = 0; j < 0xFF; j++){
        //         bytes1 data = bytes1(j);
        //         if(encodedVMs[0][i] == data){
        //             continue; // same byte, can skip
        //         }

        //         encodedVMs[0][i] = data; // Modification to see if tampering backfires.
        //         vm.expectRevert();
        //         wormholeEndpointChain2.receiveMessage(encodedVMs[0]);
        //         encodedVMs[0] = copyBytes(oldEncodedVM0); // Fix it!
        //     }
        // }
        // encodedVMs[0] = oldEncodedVM0;
        ...
        vm.startPrank(userC);
        vm.expectRevert(); // Allowance check for the burn. 
    ...
    function test_baseRelayer() public {
        ...
        // Setting up the transfer with some tokens
        DummyToken token1 = DummyToken(managerChain1.token());
        DummyToken token2 = DummyToken(managerChain2.token());
        uint8 decimals = token1.decimals();
        uint256 sendingAmount = 5 * 10 ** decimals; 
        ...
        uint256 managerBalanceBefore = token1.balanceOf(address(managerChain1));
        uint256 userBalanceBefore = token1.balanceOf(address(userA));
        managerChain1.transfer( // Breaking on 'quoteDeliveryPrice' within '0xba90A30aef27a4f644fdd30B4D5122120e4f6C17'. The contract appears to be completely blank besides being deployed. Everything is set to 0s, including the owner and chain id.
            sendingAmount,
            chainId2,
            bytes32(uint256(uint160(userB))),
            false
   ...
   function test_playground() public{
...

        {
            ...
            managerChain1.setInboundLimit(NormalizedAmount.wrap(0).denormalize(18), chainId2);
            wormholeEndpointChain1.receiveMessage(encodedVMs[0]);

            vm.warp(vm.getBlockTimestamp() + 100000);
            managerChain1.completeInboundQueuedTransfer(0x86414038c8494b37f205d58f2d14b7328167c79ed3308140b45548dc1776ab27);
    ...
    function test_multi_endpoint() public {
        vm.chainId(chainId1);
        vm.roll(1);
        ...
        console.log("Encoded VM Length:", encodedVMs.length);
        ...
        {
            // vm.stopPrank();
            // managerChain2.setThreshold(1); // should changing the threshold lead to a revert of incoming messages later? I don't feel like it should?
            uint256 supplyBefore = token2.totalSupply();
